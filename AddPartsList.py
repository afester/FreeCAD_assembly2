...
from AssemblyUtils import findUnusedObjectName
...
class AddPartsList:
    ...
    def Activated(self):
        if not drawing_dimensioning_installed:
            QtGui.QMessageBox.critical( QtGui.qApp.activeWindow(), 'Drawing Dimensioning workbench required', 
                                        'The Parts list feature requires the Drawing Dimensioning workbench<br>(<a href="https://github.com/hamish2014/FreeCAD_drawing_dimensioning">https://github.com/hamish2014/FreeCAD_drawing_dimensioning</a>).<br>Release from 12 April 2016 or later required.' )
            return
        ...
        for obj in FreeCAD.ActiveDocument.Objects:
            # TODO: This ONLY considers imported parts, not standard parts fetched e.g. from the Bolts library! 
            if 'importPart' in obj.Content:
                Logging.debug('adding %s to parts list' % obj.Name)
                P.addObject( PartListEntry(obj) )

        d.partsList = P
        for pref in d.preferences:
            pref.dimensioningProcess = d #required to be compatible with drawing dimensioning
    ...
    def GetResources(self):
        return {
            'Pixmap'  : ':/assembly/icons/addPartsList.svg' ,
            'MenuText': 'Create parts list',
            'ToolTip' : 'Create a parts list from the objects imported using the Assembly workbench'
            }
...
class PartsListTaskDialog:
    def __init__(self):
        ...
        self.form = FreeCADGui.PySideUic.loadUi( ':/assembly/ui/PartsList.ui' )
        self.form.setWindowIcon(QtGui.QIcon( ':/assembly/icons/addPartsList.svg' ) )
    ...
    def setDefaults(self):
        ...
        parms.SetFloat('padding',                 form.doubleSpinBox_padding.value()                 )
