...
import Logging
from ConstraintSystems import FixedObjectSystem, getSubElementAxis, getSubElementPos 

from AssemblyUtils import planeSelected, cylindricalPlaneSelected, circularEdgeSelected, linearEdgeSelected, vertexSelected, sphericalSurfaceSelected
...
class SolverCache:
    ...
    def retrieve( self, rootSystem, constraintObjectQue, prepare_for_record=True):
        ...
        if rootParameters(rootSystem) != self.rootParameters:
            Logging.debug('cache: rootParameters(rootSystem) != self.rootParameters')
            return rootSystem, 0
...
def copy_constraint_system( sys ):
    ...
    try:
        ...
        Logging.debug('******** cache copy error: %s' % msg)
        Logging.debug('trying to determine where the object causing the crash is')
        for node in tree:
            for attr_name in node.__dict__.keys():
                attr =  getattr(node,attr_name)
                if str( type( attr ) ) == "<type 'FeaturePython'>":
                    Logging.debug("  %s.%s is <type 'FeaturePython'>" % (node,attr_name))
                if str(type(attr)) == "<type 'App.Document'>":
                    Logging.debug("  %s.%s is <type 'App.Document'>" % (node,attr_name))
                if hasattr(attr,'__dict__'):
                    for sub_attr_name in attr.__dict__.keys():
                        sub_attr =  getattr(attr,sub_attr_name)
                        if str( type( sub_attr ) ) == "<type 'FeaturePython'>":
                            Logging.debug("  %s.%s.%s is <type 'FeaturePython'>" % (node, attr_name, sub_attr_name ))
                        if str( type( sub_attr ) ) == "<type 'App.Document'>":
                            Logging.debug("  %s.%s.%s is <type 'App.Document'>" % (node, attr_name, sub_attr_name ))
