import Logging
from ConstraintTools import solve_via_Newtons_method, GradientApproximatorForwardDifference
from AssemblyUtils import getSubElementPos, getSubElementAxis, formatDictionary
from Lib3d import axis_to_azimuth_and_elevation_angles, normalize, plane_degrees_of_freedom, rotation_required_to_rotate_a_vector_to_be_aligned_to_another_vector
from Lib3d import axis_rotation_matrix, distance_between_axis_and_point, planeIntersection
from DegreesOfFreedom import PlacementDegreeOfFreedom, AxisRotationDegreeOfFreedom, LinearMotionDegreeOfFreedom
import numpy
from numpy import dot, pi, cross, cos
...
class ConstraintSystemPrototype:
    ...
    def __init__(self, parentSystem, variableManager, constraintObj, constraintValue ):
        ...
        # if debugPrint.level >= 4: dp(
        Logging.debug('%s - sys2 %s' % (self.label, self.sys2.str()))
        self.init2()
        self.solveConstraintEq()
        Logging.debug('  resulting system:\n%s' % self.str(indent=' '*4, addDOFs=False)) # debugPrint.level>3))
            ...
            if len(self.solveConstraintEq_dofs) == 0:
                ...
                if self.analyticalSolution(): #if analytical solution then will update X
                    ...
                    if abs( self.constraintEq_value(self.variableManager.X) ) > tol:
                        Logging.debug('  **numerical round off error in analytical solution repeating')
                        self.analyticalSolution()
                else: #numerical solution
                    Y0 =      [ d.getValue() for d in self.solveConstraintEq_dofs ]
                    Logging.debug('%s: attempting to find solution numerically' % (self.str()))
                    yOpt = solve_via_Newtons_method(
                        ...
                        lineSearchIt=10,
                        debugPrintLevel=4, #debugPrint.level-2-PLO,
                        printF= lambda txt: Logging.debug(txt), # debugPrint(2, txt ),
    ...
    def constraintEq_f( self, Y ):
        ...
        PLO = 0 if not self.childSystem else 1 #print level offset
        Logging.debug('constraintEq_f, X %s, f(X) %s' % (self.variableManager.X,f_X))
        return f_X
    ...
    def generateDegreesOfFreedomNumerically(self ):
        Logging.debug('  attempting to generate new degrees-of-freedom numerically')
        ...
        if len(D) == 0:
            ...
            if all(df_dy == 0):
                Logging.debug('  generateDegreesOfFreedomNumerically, all(df_dy == 0), so assuming constraint is reduntant.')
                ...
                if len(df_dy) - sum(df_dy == 0) == 1:
                    Logging.debug('  generateDegreesOfFreedomNumerically, len(df_dy) - sum(df_dy == 0) == 1, removing dof with gradient != 0')
                    removeInd = list(df_dy == 0).index(False)
                elif len(df_dy) - sum(abs(df_dy) < max(abs(df_dy))*1e-6) == 1:
                    Logging.debug('  generateDegreesOfFreedomNumerically, len(df_dy) - sum(abs(df_dy) < max(abs(df_dy))*1e-6) == 1, removing dof with largest gradient')
                    removeInd = list( abs(df_dy) == max(abs(df_dy)) ).index(True)
                if removeInd != None:
                    self.generateDegreesOfFreedomNumerically_case = 0
                    Logging.debug('    removing %s' % D[removeInd])
                    self.degreesOfFreedom = [ d for i,d in enumerate(D) if i != removeInd ]
                    return
                else:
                    Logging.debug('  generateDegreesOfFreedomNumerically, trivial reductions failed; attempting to determine non-perfect DOF by trail and error.')
                    ...
                    Logging.debug('    len(active_D) %i, len(dormant_D) %i' % (len(active_D),len((dormant_D))))
                    ...
                    for i in range(len(active_D)):
                        ...
                        try:
                            ...
                            Logging.debug('unable to solve system after the locking the first %i DOFs, therefore passing through %i/%i of active_DOF' % (i+1,i,len(active_D)))
                            for j in range(i+1):
...
class AxisAlignmentUnion(ConstraintSystemPrototype):
    ...
    def analyticalSolution(self):
        ...
        for objName in [self.obj1Name, self.obj2Name]:
            ...
            if len(matches) == 3:
                Logging.debug('%s analyticalSolution available: %s has free rotation.'% (self.label, objName))
                vM = self.variableManager
                ...
                Logging.debug('    v %s, v_ref %s, directionConstraintFlag %s' % (v, v_ref, self.constraintValue))
                ...
                Logging.debug('    analyticalSolution:  axis %s, angle %s.'% (axis, angle))
                ...
                Logging.debug('  %s-%s analyticalSolution possibly available, v_angle %f, v_ref_angle %f'% (self.label, objName, v_angle, v_ref_angle))
                ...
                if error < 10**-9:
                    Logging.debug('    %s-%s  analyticalSolution solution worked, error %e'% (self.label, objName, error))
                    return True
                else:
                    Logging.debug('    %s-%s  analyticalSolution solution failed, error %e'% (self.label, objName, error))
                #d = matches[0]
    ...
    def analyticalSolutionAdjustAngle( self, angle, axis, v, v_ref ):
        ...
        v_rotated = dot( axis_rotation_matrix( angle, *axis), v)
        ax_prod = dot( v_rotated, v_ref )
    ...
    def generateDegreesOfFreedomAnalytically( self ):
        ...
        for objName in [self.obj1Name, self.obj2Name]:
            ...
            if len(matches) == 3:
                Logging.debug('%s Logic "%s": reducing from 3 to 1 rotational degree of freedom (2 rotation degrees fixed in defining axis of rotation)' % (self.label, objName))
                ...
                if 1 - abs(dot(a, matches[0].axis)) < self.solveConstraintEq_tol: #
                    Logging.debug('%s Logic "%s": AxisRotationDegreeOfFreedom with same axis already exists not reducing dofs for part' % (self.label, objName))
                    self.degreesOfFreedom_updateInd = -1
                    self.degreesOfFreedom = dofs
                    success = True
                else:
                    Logging.debug('%s Logic "%s": 2 different rotation axis specified, therefore fixing rotation (0 rotational degrees of freedom)' %
 (self.label, objName))
                    self.degreesOfFreedom = [ d for d in dofs if not d in matches ]
                    self.degreesOfFreedom_updateInd = -1
                    success = True
                ...
                Logging.debug('%s Logic "%s": no rotational degrees of freedom ignoring.' % (self.label, objName))
                #    self.degreesOfFreedom = dofs
                #    self.degreesOfFreedom_updateInd = -1
                #    success = True
        if len(matches) > 0 and self.constraintValue == "none": #then assign direction, to make users and solvers life easier, #outside loop due to break
            ...
            self.constraintValue = "aligned"  if dot( a,b ) > 0 else "opposed"
...
class LockRelativeAxialRotationUnion(ConstraintSystemPrototype):
    ...
    def generateDegreesOfFreedomAnalytically( self ):
        ...
        if len(matches) == 1:
            Logging.debug('%s Logic: removing axis rotation dof of %s' % (self.label, self.objectToLock))
            self.degreesOfFreedom = [ d for d in D if not d in matches ]
        else:
            self.degreesOfFreedom = D
            Logging.debug('%s Logic Failure, unable to remove relative rotation degree of freedom' % (self.label))
        return True
...
class AddFreeObjectsUnion(ConstraintSystemPrototype): #for adding free objects to parent system which are not constrainted to the parent system
    ...
    def __init__(self, parentSystem, variableManager, constraintObj ):
        ...
        parentSystem.childSystem = self
        self.solveConstraintEq()
        Logging.debug('AddFreeObjectsUnion resulting system:\n%s' % self.str(indent=' '*4, addDOFs=4))  # debugPrint.level>3))
...
class VertexUnion(ConstraintSystemPrototype):
    ...
    def analyticalSolution(self):
        ...
        for objName in [self.obj1Name, self.obj2Name]:
            ...
            if len(matches) > 0 :
                Logging.debug('    %s %s has linear displacement degrees of freedom, checking for analyticalSolution.'% (self.label, objName))
                vM = self.variableManager
                ...
                Logging.debug('    requiredDisp %s' % requiredDisp )
                ...
                if norm( requiredDisp - actualDisp) < 10**-9:
                    Logging.debug('    %s analyticalSolution available by moving %s.'% (self.label, objName))
                    for m,v in zip(matches,V):
    ...
    def generateDegreesOfFreedomAnalytically( self ):
        ...
        for objName in [self.obj1Name, self.obj2Name]:
            ...
            if len(matches) == 3: #todo somehow add support for 3,2 cases
                Logging.debug('  VertexUnion Logic: %s removing all 3 movement degrees of freedom' % objName )
                self.degreesOfFreedom = [ d for d in D if not d in matches ]
                success = True
                break
        return success
...
class AxisDistanceUnion(ConstraintSystemPrototype):
    ...
    def constraintEq_value( self, X ):
        ...
        if numpy.isnan(dist):
            Logging.debug('numpy.isnan(dist)')
            Logging.debug('  locals %s' % formatDictionary(locals(),' '*6) )
            Logging.debug('  %s.__dict %s' % (self.label, formatDictionary( self.__dict__,' '*6 ) ) )
        ...
        if  self.constraintValue == 0:
            ...
            for objName in [self.obj1Name, self.obj2Name]:
                ...
                if len(matches) > 0:
                    ...
                    pos1 = vM.rotateAndMove( self.obj1Name, self.pos1_r, vM.X )
                    pos2 = vM.rotateAndMove( self.obj2Name, self.pos2_r, vM.X )
                    error_v =  (pos1-pos2) - dot(a,pos1-pos2)*a
    ...
    def generateDegreesOfFreedomAnalytically( self ):
        ...
        for objName in [self.obj1Name, self.obj2Name]:
            ...
            if len(matches) == 3:
                Logging.debug('%s Logic: %s - reducing linear displacement degrees of freedom from 3 to 1' % (self.label, objName))
                self.degreesOfFreedom = [ d for d in dofs if not d in matches ]
                ...
                c = cross( matches[0].directionVector, matches[1].directionVector)
                planeNormalMatches = c/norm(c)
                if abs(dot( axisVector, planeNormalMatches)) < 10 **-6: #then co-planar
                    Logging.debug('%s Logic: %s axis in movement plane, therefore linear degrees of freedom reduced from 2 to 1' % (self.label, objName))
                    self.degreesOfFreedom = [ d for d in dofs if not d in matches ]
                    self.degreesOfFreedom.append( LinearMotionDegreeOfFreedom( self, objName) )
                    self.dof_added = True
                    success = True
                    ...
                    Logging.debug('%s Logic: %s axis not in movement plane, therefore linear degrees of freedom reduced from 2 to 0' % (self.label, objName))
                    self.degreesOfFreedom =  [ d for d in dofs if not d in matches ]
                    success = True
                    break
            elif len(matches) == 1:
                if abs(dot(axisVector , matches[0].directionVector)) < 10 **-6: #then constraint redudant
                    Logging.debug('%s Logic: %s - axis movement constraint does not effect remaining dof -> no dof reduction.' % (self.label, objName))
                    #self.degreesOfFreedom =  dofs
                    #success = True
                else:
                    Logging.debug('%s Logic: %s - axis movement constraint different from last linear displacement degree of freedom -> reducing degrees of freedom from 1 to 0' % (self.label, objName))
                    self.degreesOfFreedom = [ d for d in dofs if not d in matches ]
                    success = True
                    break
...
class PlaneOffsetUnion(ConstraintSystemPrototype):
    ...
    def constraintEq_value( self, X ):
        ...
        pos1 = vM.rotateAndMove( self.obj1Name, self.pos1_r, X )
        pos2 = vM.rotateAndMove( self.obj2Name, self.pos2_r, X )
        dist = dot(a, pos1 - pos2) #distance between planes
        return dist - self.constraintValue
            ...
            if len(matches) > 0:
                ...
                for j in reversed(range(len(matches))):
                    if abs( dot(a, matches[j].directionVector) ) < 10**-9:
                        del matches[j]
                if len(matches) == 0 :
                    Logging.debug('    %s %s aborting analytical solution, since dof perpindicular to required displacement.'% (self.label, objName))
                    continue
                Logging.debug('    %s analyticalSolution available by moving %s.'% (self.label, objName))
                pos1 = vM.rotateAndMove( self.obj1Name, self.pos1_r, vM.X )
                pos2 = vM.rotateAndMove( self.obj2Name, self.pos2_r, vM.X )
                error =  dot(a, pos1 - pos2) - self.constraintValue
                if objName == self.obj1Name:
                    error = -error
    ...
    def generateDegreesOfFreedomAnalytically( self ):
        ...
        for objName in [self.obj1Name, self.obj2Name]:
            matches = [d for d in dofs if d.objName == objName and not d.rotational() ]
            if len(matches) == 3:
                Logging.debug('PlaneOffsetUnion Logic: %s - reducing linear displacement degrees of freedom from 3 to 2' % objName)
                self.degreesOfFreedom = [ d for d in dofs if not d in matches ]
                self.degreesOfFreedom.append( LinearMotionDegreeOfFreedom( self, objName) )
                self.degreesOfFreedom.append( LinearMotionDegreeOfFreedom( self, objName) )
                ...
                if norm(planeNormalVector - planeNormalMatches) < 10 **-6: #then constraint redudant
                    Logging.debug('PlaneOffsetUnion Logic: %s - plane constraint with normal already exist, not reducing dofs for part' % objName)
                    #self.degreesOfFreedom =  dofs
                    #self.dofs_removed = []
                    #success = True
                else:
                    Logging.debug('PlaneOffsetUnion Logic: %s - reducing linear displacement degrees of freedom from 2 to 1' % objName)
                    self.degreesOfFreedom = [ d for d in dofs if not d in matches ]
                    self.degreesOfFreedom.append( LinearMotionDegreeOfFreedom( self, objName ) )
                    self.dofs_removed = matches
                    success = True
                ...
                vM = self.variableManager
                planeNormalVector = vM.rotate( self.obj1Name, self.a1_r, vM.X )
                if abs(dot( planeNormalVector, matches[0].directionVector)) < 10 **-6: #then constraint redudant
                    Logging.debug('PlaneOffsetUnion Logic: %s - planeNormal constraint does not effect remaining dof -> no dof reduction.' % objName)
                    #self.degreesOfFreedom =  dofs
                    #self.dofs_removed = []
                    #success = True
                else:
                    Logging.debug('PlaneOffsetUnion Logic: %s - reducing linear displacement degrees of freedom from 1 to 0' % objName)
                    self.degreesOfFreedom = [ d for d in dofs if not d in matches ]
                    self.dofs_removed = matches
                    success = True
                    break
    ...
    def updateDegreesOfFreedomAnalytically( self ):
        ...
        if len(self.dofs_removed) == 3:
            ...
            c = cross( self.dofs_removed[0].directionVector, self.dofs_removed[1].directionVector)
...
class AngleUnion(AxisAlignmentUnion):
    ...
    def constraintEq_value( self, X ):
        ...
        a = vM.rotate( self.obj1Name, self.a1_r, X )
        b = vM.rotate( self.obj2Name, self.a2_r, X )
        return cos(self.constraintValue) - dot( a,b )
        # for another day
        #c = crossProduct( a, b)
        #if norm(c) > 0:
        #    axis = normalize(c)
